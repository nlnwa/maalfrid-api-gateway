...
function detectAndUpdate(cursor) {
  ...
  return cursor.eachAsync((row) => {
    ...
    return detectLanguage(row.text)
      ...
      .catch((err) => console.log('detectLanguage failed for warcId: ', row.warcId, err.message));
  }, {concurrency: 10});
}
...
module.exports = {
  ...
  async generateReport(ctx) {
    ...
    const cursor = await r.table('executions').map(r.row('id')).run({cursor: true});
    ...
    cursor.eachAsync((id) => {
      ...
      // Get a selection of warcId's from the crawl log with recordType response and revisit
      const selection = db.getCrawlLogAndCorrespondingExtractedText(id);
      r.db('report').table('cache').insert({
        id,
        stat: db.getLanguageStatistic(selection)
      }).run();
