...
module.exports = {
  async detectLanguageOfAllTexts(ctx) {
    let step = 50;
    const count = parseInt(db.getExtractedTexts().count().run(), 10);
    debug('(detectLanguageOfAllTexts) count=%d', count);
    for (let i = 0; i < count; i += step) {
      if (i + step > count) {
        step = count - i + 1;
      }
      const texts = db.getExtractedTexts().slice(i, i + step).run();
      await updateLanguageCodes(texts);
      debug('(updateLanguageCodes): %d-%d\n', i, i + step);
    }
    ctx.body = 'Detection finished';
  },
