 ...
 */
function getStatistics (startTime, endTime, entityId) {
  let query = r.db('maalfrid').table('statistics')
  if (endTime && startTime) {
    query = query.between(startTime, endTime, { index: 'endTime' })
  }
  if (entityId) {
    query = query.filter({ entityId })
  }
  return query.run()
}
