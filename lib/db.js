...
module.exports = {
   ...
   * @return {*} write query
   */
  async generateAggregate(since) {
    try {
      await r.db('maalfrid').tableDrop('tmp');
    } catch (err) {
      // noop - discard ReqlRuntimeError if database not exists
    } finally {
      await r.db('maalfrid').tableCreate('tmp');
    }

    const seedIds = r.db('maalfrid').table('seeds')('id').coerceTo('array').run()
    const cursor = await r.db('veidemann').table('executions')
      .filter()
      .getAll(r.args(seedIds), {index: 'seedId'}).run({cursor: true});

    await cursor.eachAsync(async (execution) => {
      return r.db('maalfrid').table('tmp')
        .insert(
          r.db('veidemann').table('crawl_log').getAll(r.args([execution.id]), {index: 'executionId'})
            .without(
              'fetchTimeMs',
              'fetchTimeStamp',
              'timeStamp',
