...
module.exports = {
  ...
  getExtractedText(warcId) {
    return r.db('veidemann').table('extracted_text').get(warcId).getField('text');
  },
  ...
  getStats(executionId) {
    return r.db('maalfrid').table('stats').get(executionId)('stat');
  },
  ...
  getExecutions(jobId, seedId, startTime, endTime) {
    return r.db('maalfrid').table('aggregate')
      ...
      .pluck('startTime', 'endTime', 'id', 'jobId');
  },
};
